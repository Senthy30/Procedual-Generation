...
public class DayNightCycle : MonoBehaviour {
    ...
    private float lastValueForUpdate, lastValueForUpdateWater;
    ...
    void UpdateValues(float currentMomentInTransition) {
        ...
        nightLight.intensity = lightningCurve.Evaluate(1 - currentMomentInTransition) * 0.15f;
        skyboxMaterial.SetFloat("_AtmosphereThickness", skyboxCurve.Evaluate(currentMomentInTransition));

        if (Mathf.Abs(currentMomentInTransition - lastValueForUpdateWater) >= 0.03f) {
            GameObject waterObject;
            int length = EndlessTerrain.visibleTerrainChunks.Count;
            for (int i = 0; i < length; i++) {
                waterObject = EndlessTerrain.visibleTerrainChunks[i].waterObject;
                if (waterObject != null) 
                    waterObject.GetComponent<Renderer>().material.SetFloat("Vector1_6269b1025b26473ca8bc61634f34b537", smoothnessCurve.Evaluate(currentMomentInTransition));
            }

            lastValueForUpdateWater = currentMomentInTransition;
        }
    }
